...
def getBboxNorm(pcdpath):
    scale_mat = np.eye(4)
    pcd = o3d.io.read_point_cloud(pcdpath)
    bbox = pcd.get_axis_aligned_bounding_box()
    scale = bbox.get_extent().max() / 2
    meanvec = bbox.get_center()
    ...
    radius = scale * 1.05
    translate = -meanvec
    return {"translate": translate, "radius": radius}, bbox
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder,masks_folder=None):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        #     mask_path = os.path.join(masks_folder,f'blockmask_{extr.name.split(".")[0]}.jpg')
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX,
                              ...
                              width=width, height=height, fx=focal_length_x, fy=focal_length_y, mask_path=None)
...
def readColmapSceneInfo(path, images, eval, llffhold=8,meta=None):
    try:
    ...
    if train_list is not None:
        ...
        train_cam_infos = cam_infos #[c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]
    ...
    has_points = False
    has_bbox = False
    if os.path.exists(os.path.join(path, "points.ply")):
        has_points = True
    if os.path.exists(os.path.join(path, "bbox.ply")):
        has_bbox = True



    if has_bbox:
        nerf_normalization, aabb = getBboxNorm(os.path.join(path,"bbox.ply"))
    elif has_points:
        nerf_normalization, aabb = getBboxNorm(os.path.join(path, "points.ply"))
    else:
        nerf_normalization = getNerfppNorm(train_cam_infos)

    if meta is not None:
        nerf_normalization, aabb = getBboxNorm(os.path.join(meta['initply']))

    print(f'has_bbox: {has_bbox}, has points: {has_points}')
    ...
    if meta is not None:
        ply_path = os.path.join(meta['initply'])
    ...
    if not os.path.exists(ply_path):
        ...
        if has_bbox:
            valid_mask = (xyz >= aabb.min_bound) * (xyz <= aabb.max_bound)
            valid_mask = valid_mask.sum(axis=-1) >= 3
            xyz = xyz[valid_mask]
            rgb = rgb[valid_mask]
    ...
    try:
        ...
        try:
            ...
            pcd = BasicPointCloud(points=pcd.vertices, colors=pcd.colors[:,:3].astype(np.float32)/255, normals=None)
        except:
            pcd = None
    ...
    if pcd is None :
        if has_points:
            if meta is not None:
                pcd = o3d.io.read_point_cloud(os.path.join(meta['initply']))
            else:
            ...
            if len(pcd.colors) > 0:
                ...
                num_pts = xyz.shape[0]
                pcd = BasicPointCloud(points=pcd.points,
                                      colors=pcd.colors.astype(np.float32) / 255, normals=None)
                storePly(ply_path, xyz, pcd.colors * 255)
            else:
                print(f"Generating random color for point cloud...")
                # We create random points inside the bounds of the synthetic Blender scenes
                xyz = np.asarray(pcd.points)
                num_pts = xyz.shape[0]
