...
class DPTHead_pathway(nn.Module):
    def __init__(
            self,
            in_channels,
            features=256,
        ...
        super(DPTHead_pathway, self).__init__()
        ...
        self.scratch.output_conv2 = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Conv2d(head_features_2, channel_out, kernel_size=1, stride=1, padding=0),
            nn.Sigmoid()
        )

    def forward(self, out_features, out_features_pathway, patch_h, patch_w):
        out = []
        assert self.use_clstoken==False, "config err!"
        for i, x in enumerate(zip(out_features, out_features_pathway)):
            x1,x2 = x[0][0], x[1][0]
            x = torch.cat((x1, x2), -1)
