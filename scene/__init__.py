...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0],datameta=None):
        """b
        ...
        self.imglist = os.listdir(args.source_path + '/' + args.images)
        self.imglist = [f.split('.')[0] for f in self.imglist]

        self.imglist.sort()

        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval,meta=datameta)
