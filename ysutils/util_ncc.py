...
def calncc_patch(src,gt,patch_size=5):
    total_patch_size = (patch_size * 2 + 1) ** 2

    if len(gt.shape) == 2: #chw
        gt_image_gray = torch.tensor(gt).unsqueeze(0).float().cuda()
        src_image_gray = torch.tensor(src).unsqueeze(0).float().cuda()

    else:
        gt_image_gray = torch.tensor(gt).float().cuda()
        src_image_gray = torch.tensor(src).float().cuda()

    H, W = gt_image_gray.squeeze(0).shape
    ...
    pixels_patch_src = ori_pixels_patch.clone()
    pixels_patch_src[:, :, 0] = 2 * pixels_patch_src[:, :, 0] / (W - 1) - 1.0
    pixels_patch_src[:, :, 1] = 2 * pixels_patch_src[:, :, 1] / (H - 1) - 1.0
    src_gray_val = F.grid_sample(src_image_gray.unsqueeze(1), pixels_patch_src.reshape(1, -1, 1, 2), align_corners=True)
    src_gray_val = src_gray_val.reshape(-1, total_patch_size)

    ## compute loss
    ncc, ncc_mask = lncc(ref_gray_val, src_gray_val)
