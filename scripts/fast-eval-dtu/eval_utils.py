...
def comput_one_scan_cuda(scanid,            # the scan id to be computed 
    ...
    transform_npz = kargs['args'].transform_2dgs
    if transform_npz is not None:
        cam_file = transform_npz
        scale_mat = np.identity(4)
        if os.path.exists(cam_file):
            camera_param = dict(np.load(cam_file))
            scale_mat = camera_param['scale_mat_0']
        else:
            raise ValueError(cam_file)
        pcd_np = pcd_np* scale_mat[0, 0] + scale_mat[:3, 3][None]
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(pcd_np)
        o3d.io.write_point_cloud(os.path.join(out_dir,'transform_dtu.ply'),pcd)
    ...
    dist_d2s_in = dist_d2s[grid_inbound][in_obs]
    dist_s2d_in = dist_s2d[above]

    mean_d2s = dist_d2s_in[dist_d2s_in < max_dist].mean()
    mean_s2d = dist_s2d_in[dist_s2d_in < max_dist].mean()

    mean_d2s_half = dist_d2s_in[dist_d2s_in < max_dist/2].mean()
    mean_s2d_half = dist_s2d_in[dist_s2d_in < max_dist/2].mean()

    mean_d2s_quart = dist_d2s_in[dist_d2s_in < max_dist/4].mean()
    mean_s2d_quart = dist_s2d_in[dist_s2d_in < max_dist/4].mean()

    pbar.update(1)
    pbar.set_description(f'[scan{scanid}] visualize error')

    save_pcd = o3d.geometry.PointCloud()
    save_pcd.points = o3d.utility.Vector3dVector(pcd.cpu().numpy())
    o3d.io.write_point_cloud(os.path.join(out_dir,'final_rating.ply'),save_pcd)
    # if vis:
    #     Path(out_dir).mkdir(parents=True, exist_ok=True)
    #     vis_dist = vis_thresh
    #     R = np.array([[255,0,0]], dtype=np.float64)
    ...
    #     data_alpha = dist_d2s.clip(max=vis_dist) / vis_dist
    #
    #     inbound_cpu = inbound.cpu().numpy()
    #     in_obs_cpu = in_obs.cpu().numpy()
    #     grid_inbound_cpu = grid_inbound.cpu().numpy()
    #     data_alpha_cpu = data_alpha.cpu().numpy()
    #     dist_d2s_cpu = dist_d2s.cpu().numpy()
    #     stl_cpu = stl.cpu().numpy()
    #     dist_s2d_cpu = dist_s2d.cpu().numpy()
    #
    #     data_color[ np.where(inbound_cpu)[0][grid_inbound_cpu][in_obs_cpu] ] = R * data_alpha_cpu + W * (1-data_alpha_cpu)
    #     data_color[ np.where(inbound_cpu)[0][grid_inbound_cpu][in_obs_cpu][dist_d2s_cpu[:,0] >= max_dist] ] = G
    ...
    pbar.close()
    over_all = (mean_d2s + mean_s2d) / 2
    over_all_half = (mean_d2s_half + mean_s2d_half) / 2
    over_all_quart = (mean_d2s_quart + mean_s2d_quart) / 2
    # print(f"\t\t\tacc.(mm):{mean_d2s:.4f}, comp.(mm):{mean_s2d:.4f}, overall(mm):{over_all:.4f}")
    return (mean_d2s.cpu().numpy(), mean_s2d.cpu().numpy(), over_all.cpu().numpy(), mean_d2s_half.cpu().numpy(), mean_s2d_half.cpu().numpy(),
            over_all_half.cpu().numpy(), mean_d2s_quart.cpu().numpy(), mean_s2d_quart.cpu().numpy(), over_all_quart.cpu().numpy())
