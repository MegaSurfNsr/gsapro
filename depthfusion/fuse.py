...
def load_data(cam_path, helixout_path, image_path, prior_mask=None,rgb_flag=False,extra=False):
	'''
	...
	for i in tqdm.tqdm(range(len(rgb_paths))):
		...
		dep_map = -1 * helixout[:, :, -1] * intr_mat[0,0] / ((raw_grid[1] - intr_mat[0, 2])*helixout[:,:,0] + (intr_mat[0,0]/intr_mat[1,1]) * (raw_grid[0] - intr_mat[1,2]) * helixout[:,:,1] + intr_mat[0,0] * helixout[:,:,2])
		depths.append(torch.from_numpy(dep_map).unsqueeze(0))
		norm = helixout[:,:,:3]
		norm = norm / np.linalg.norm(norm,axis=-1,keepdims=True)
		norms.append(torch.from_numpy(np.linalg.pinv(extr_mat[:3,:3])[None,None,:,:] @ norm[:,:,:,None]).squeeze(-1))
		if rgb_flag:
		...
		if prior_mask is not None:
			...
			h_rgb,w_rgb = prior_raw.shape
			if h_rgb != h:
				prior_raw = cv2.resize(np.asarray(prior_raw,dtype=np.uint8),(w,h))
			prior_masks.append(np.asarray(prior_raw,dtype=bool))
	...
	if extra:
		return depths, projs, rgbs, confs, norms, prior_masks,intrins,extrins
	else:
...
def main():
	# args.dist_thresh = 0.5
	# args.dist_thresh_downsample = 0.5
	# args.num_consist = 5
	...
	depths, projs, rgbs, confs, norms, prior_masks,intrin_mats,extrin_mats = load_data(args.cam_path, args.helixout_path, args.image_path,args.prior_mask, args.extract_color,extra=True)
	...
	for i in tqdm.tqdm(range(tot_frame)):
		...
		if args.extract_color:
			...
			# o3d.io.write_point_cloud(os.path.join(args.save_path, 'test.ply'), pcd)
	...
	print('down sampling')
	# pcd = pcd.voxel_down_sample(voxel_size=args.dist_thresh_downsample)
	o3d.io.write_point_cloud('{}/{}.ply'.format(args.save_path, f'{args.prefix}fusion_downsample_test'), pcd, write_ascii=False)
