...
class GaussianModel:
    ...
    def densification_postfix(self, new_xyz, new_knn_f, new_features_dc, new_features_rest, new_opacities, new_scaling,
        ...
        #     [self.max_radii2D, torch.zeros([self.get_xyz.shape[0] - self.max_radii2D.shape[0]], device="cuda")])
        # self.max_radii2D = torch.concat([self.max_radii2D, torch.zeros((new_xyz.shape[0]), device="cuda") ], dim=0)
    ...
    def add_densification_stats(self, viewspace_point_tensor, viewspace_point_tensor_abs, update_filter):
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter, :2], dim=-1,
                                                             keepdim=True)
        self.xyz_gradient_accum_abs[update_filter] += torch.norm(viewspace_point_tensor_abs.grad[update_filter, :2],
                                                                 dim=-1, keepdim=True)
