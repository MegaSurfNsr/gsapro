...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background,
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        # cv2.imwrite(os.path.join(render_depth_path, view.image_name + ".tiff"), depth)
        ...
        # cv2.imwrite(os.path.join(render_normal_path, view.image_name + ".tiff"), normal)
        save_mat = np.concatenate([depth[:, :, None], normal], axis=2)
        os.makedirs(os.path.join(render_depth_path,'pgsr_prior'),exist_ok=True)
        np.save(os.path.join(render_depth_path,'pgsr_prior', view.image_name),np.asarray(save_mat,dtype=np.float32))
    ...
    if volume is not None:
        ...
        for idx, view in enumerate(tqdm(views, desc="TSDF Fusion progress")):
            ...
            # color = np.asarray(view.original_image)
            color = o3d.geometry.Image(np.ascontiguousarray(view.original_image.permute(1,2,0)*255,dtype=np.uint8))
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
    ...
    with torch.no_grad():
        ...
        color_type=o3d.pipelines.integration.TSDFVolumeColorType.NoColor)#o3d.pipelines.integration.TSDFVolumeColorType.__dict__['None']RGB8
