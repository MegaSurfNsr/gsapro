...
if __name__ == '__main__':
    ...
    parser.add_argument('--transform_mat_dir', type=str, default=None)
    args = parser.parse_args()

    args.scan = int(args.scan_id[4:])
    ...
    transform_npz = args.transform_mat_dir
    if transform_npz is not None:
        cam_file = os.path.join(transform_npz, args.scan_id,'cameras.npz')
        ...
        data_pcd = data_pcd* scale_mat[0, 0] + scale_mat[:3, 3][None]
        pcd_transform = o3d.geometry.PointCloud()
        pcd_transform.points = o3d.utility.Vector3dVector(data_pcd)
        o3d.io.write_point_cloud(os.path.join(args.vis_out_dir,'transform_dtu.ply'),pcd_transform)
    ...
    print(time.ctime())
    nn_engine = skln.NearestNeighbors(n_neighbors=1, radius=thresh, algorithm='kd_tree', n_jobs=20)
    ...
    for max_dist in max_dist_list:
        ...
        write_vis_pcd(f'{args.vis_out_dir}/vis_{args.scan:03}_d2s_{max_dist.__str__()}.ply', data_down, data_color)
        stl_color = np.tile(B, (stl.shape[0], 1))
        stl_alpha = dist_s2d.clip(max=vis_dist) / vis_dist
        stl_color[ np.where(above)[0] ] = R * stl_alpha + W * (1-stl_alpha)
        stl_color[ np.where(above)[0][dist_s2d[:,0] >= max_dist] ] = G
        write_vis_pcd(f'{args.vis_out_dir}/vis_{args.scan:03}_s2d_{max_dist.__str__()}.ply', stl, stl_color)
        ...
        out_file = os.path.join(args.vis_out_dir, f'result_{max_dist.__str__()}.csv')
